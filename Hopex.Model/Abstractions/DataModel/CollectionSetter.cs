...
namespace Hopex.Model.Abstractions.DataModel
{
    ...
    public class CollectionSetter : ISetter
    {
        ...
        public async Task UpdateElementAsync(HopexDataModel model, IModelElement source)
        {
            ...
            switch (Action)
            {
                case CollectionAction.ReplaceAll:
                    ...
                    {
                        await ConnectToAsync(model, source, link, element, true);
                    }
                    break;
                ...
                case CollectionAction.Remove:
                    ...
                    foreach (var element in elements)
                    {
                        await ConnectToAsync(model, source, link, element, false);
                    }
                    break;
            }
        }
        ...
        private MegaObject RemoveConnection(IModelElement source, HopexModelElement elemToInsert, IPathDescription[] path)
        {
            ...
            else if (path.Any()) //remove single link
            {
                ...
                if (!CanRemoveConnection(source.IMegaObject, elemToInsert.IMegaObject))
        ...
        private void RemoveAllConnection(IModelElement source, IPathDescription[] paths)
        {
            ...
            {
                List<MegaObject> interObjects = new List<MegaObject>();
                FillInterObjects(source.MegaObject, null, paths, ref interObjects);
                interObjects.ForEach(interObject => interObject.Delete("NoHierarchy"));
            }
        ...
        protected virtual bool FillInterObjects(MegaObject source, MegaObject target, IPathDescription[] paths, ref List<MegaObject> interObjects, int idx = 0)
        {
        ...
        private async Task<IModelElement> ConnectToAsync(IHopexDataModel model, IModelElement source, IRelationshipDescription link, Dictionary<string, object> properties, bool insert)
        {
            ...
            else if (properties.ContainsKey("externalId"))
            {
                id = properties["externalId"].ToString();
                idType = IdTypeEnum.EXTERNAL;
            }
            ...
            var current = insert ? InsertConnection(source, elemToInsert, link, properties) : RemoveConnection(source, elemToInsert, link.Path);

            return current != null ? new HopexModelElement((HopexDataModel)model, elemSchema, current, id) : null;
        }
        ...
        private MegaObject InsertConnection(IModelElement source, HopexModelElement elemToInsert, IRelationshipDescription relationShip, Dictionary<string, object> propertyValues)
        {
            var current = source.MegaObject;
            ...
            for (int ix = 0; ix < path.Length; ix++)
            {
                ...
                if (isLast)
                {
                    ...
                    iCurrent = elemToInsert.IMegaObject;
                }
                else
                {
                ...
                if (ix == 0 && relationShip != null)
                {
                    UpdateLinkAttributes(elemToInsert, relationShip, propertyValues, current);
                }
        ...
        internal virtual void UpdateLinkAttributes(HopexModelElement elemToInsert, IRelationshipDescription relationShip, Dictionary<string, object> propertyValues, MegaObject current)
        {
