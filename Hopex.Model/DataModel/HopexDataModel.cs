...
namespace Hopex.Model.DataModel
{
    ...
    public class HopexDataModel : IHopexDataModel, IDisposable
    {
        ...
        public Task<IModelElement> RemoveElementAsync(IClassDescription schema, string id, IdTypeEnum idType, bool cascade)
        {
            ...
            List<MegaObject> objectsToDelete = null;
            switch (idType)
            {
                case IdTypeEnum.INTERNAL:
                    objectsToDelete = _root.GetSelection($"SELECT {schema.Id}[{schema.Name}] WHERE {MetaAttributeLibrary.AbsoluteIdentifier} = \"{id}\"").ToList();
                    if (objectsToDelete.Count() != 1)
                    {
                        throw new ExecutionError($"{schema.Name} with id: {id} not found.");
                    }
                    break;
                case IdTypeEnum.EXTERNAL:
                    objectsToDelete = _root.GetSelection($"SELECT {schema.Id}[{schema.Name}] WHERE ~CFmhlMxNT1iE[ExternalIdentifier] = \"{id}\"").ToList();
                    if (!objectsToDelete.Any())
                    {
                        throw new ExecutionError($"{schema.Name} with external id: {id} not found.");
                    }
                    break;
                default:
