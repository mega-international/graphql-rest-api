...
namespace Mega.WebService.GraphQL.Tests.Sources.Tests
{
    public abstract class AbstractTest
    {
        ...
        {
            _Requester = new GraphQLRequester($"{_myServiceUrl}/api/{(IsAsyncTest() ? "async/" : "")}{_schemaITPM}");
        }

        protected virtual bool IsAsyncTest()
        {
            return true;
        }
        ...
        public async Task<ProgressionModel> Run(ITestParam oTestParam, ProgressionModel progression)
        {
            ...
            try
            {
                ...
                _progression.SetStatus(TestStatus.Completed);
            }
            catch(Exception)
            {
                _progression.SetStatus(TestStatus.Failure);
            }

            //End of test
            _progression.SetMessageTime(timeMessageManager.BuildMessage());
            return _progression;
        }
        ...
        protected async Task<List<Field>> GetFieldsRequest(string tableName, bool input, Kind flags = Kind.All)
        {
            ...
            GraphQLResponse response = await _Requester.SendPostAsync(request, IsAsyncTest());
        ...
        protected async Task<JToken> ProcessRawQuery(string query, CancellationToken token)
        {
            ...
            GraphQLResponse response = await _Requester.SendPostAsync(request, IsAsyncTest(), token);
        ...
        protected async Task DeleteOne(string metaclassName, Tuple<string, bool> arg)
        {
            //Prepare data
            string strCascade = arg.Item2 ? "true" : "false";
        ...
        {
            _progression.AddToMessageCounts($"<p>{stepName}: {count}</p>");
        }

        protected void CountedStep(string stepName, int count, int expected)
        {
            _progression.AddToMessageCounts(ColoredMessageFromBool($"<p>{stepName}: <br>current {count} / expected: {expected}</p>", count == expected));
        }

        protected void DetailedStep(string stepDetails)
        {
            _progression.AddToMessageDetails($"<p>{stepDetails}</p>");
        }
