...
namespace Mega.WebService.GraphQL.Tests.Sources.Tests
{
    public class Test18 : AbstractTest
    {
        ...
        {
            _Requester = new GraphQLRequester($"{_myServiceUrl}/api/{(IsAsyncTest() ? "async/" : "")}{_schemaAudit}");
        }

        protected override bool IsAsyncTest()
        {
            return false;
        }
        ...
        protected override async Task StepsAsync(ITestParam oTestParam)
        {
            ...
            await ProcessActionsAsync(activities, findings, 10).ConfigureAwait(false);
        }
        ...
        protected async Task<TaskResult> ProcessActionAsync(string actionName, string id)
        {
            ...
            try
            {
                await ProcessActionFromName(actionName, id, cts.Token).ConfigureAwait(false);
