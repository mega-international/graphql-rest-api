...
*/	
	...
	/*	 
	private boolean isMetAttributeObjectAsRelationShip(String metaAttributeId) {
		boolean isRelationShip = false;
		
		HashMap<String,String> mappingMetaAttributeObjectToMetaClass = MetaAttribute.mapMetaAttributeObjectToMetaclass();		
		isRelationShip = mappingMetaAttributeObjectToMetaClass.containsKey(metaAttributeId);
				
		return isRelationShip;
	}

	
	public static MegaObject getMetaClassOfMetAttributeObjectAsRelationShip(MegaRoot megaRoot, String metaAttributeId) {
		MegaObject oMetaClass = null; 
		
		HashMap<String,String> mappingMetaAttributeObjectToMetaClass = MetaAttribute.mapMetaAttributeObjectToMetaclass();		
		String oMetaClassId = mappingMetaAttributeObjectToMetaClass.get(metaAttributeId);

		if (oMetaClassId!=null && oMetaClassId != "") {
			oMetaClass = megaRoot.getObjectFromID(oMetaClassId);
		}		
	...
	private static HashMap<String,String> mapMetaAttributeObjectToMetaclass() {
		HashMap<String,String> mappingMetaAttributeObjectToMetaClass = new HashMap<String, String>();
		
		String key,value;
		
		key = "jF1hiEhwFbLF"; // ~jF1hiEhwFbLF[Skill Level]
		value = "LD1hCqgwFX6F"; // SKill Level Value - MetaClass ID	
		mappingMetaAttributeObjectToMetaClass.put(key, value);
		
		key = "w9KvON(3FHDP"; //  ~w9KvON(3FHDP[Current Workflow Status]
		value = "P2(ofnym9rA0"; // 
		mappingMetaAttributeObjectToMetaClass.put(key, value);
		
		key = "qLaREvdOHLI7"; // ~qLaREvdOHLI7[Workflow Previous Status]
		value = "P2(ofnym9rA0"; // 
		mappingMetaAttributeObjectToMetaClass.put(key, value);	
		
		key = "2NaRYvdOHzL7"; //  ~2NaRYvdOHzL7[Workflow Current Status]
		value = "P2(ofnym9rA0"; // 
		mappingMetaAttributeObjectToMetaClass.put(key, value);	
		
		key = "(YByTkohHrGG"; // ~(YByTkohHrGG[Current State]
		value = "jXRV298DpWG1"; // State (UML) 
		mappingMetaAttributeObjectToMetaClass.put(key, value);	
		
		key = "sb2usLFgFDCI"; // ~sb2usLFgFDCI[Computed State]
		value = "jXRV298DpWG1"; // State (UML) 
		mappingMetaAttributeObjectToMetaClass.put(key, value);		
		
		key = "5daB5uJAEvZL"; // ~5daB5uJAEvZL[Next Timespot]
		value = "Qan9YiIAEXW3"; // Timespot
		mappingMetaAttributeObjectToMetaClass.put(key, value);
		
		key = "vbaBptJAELWL"; // ~vbaBptJAELWL[Previous Timespot]
		value = "Qan9YiIAEXW3"; // Timespot
		mappingMetaAttributeObjectToMetaClass.put(key, value);
		
		key = "jULwouHxCzT1"; // ~jULwouHxCzT1[Next Milestone]
		value = "ZodleDY)7Tf0"; // Milestone
		mappingMetaAttributeObjectToMetaClass.put(key, value);
		
		key = "2SLwDvHxCXX1"; // ~2SLwDvHxCXX1[Previous Milestone]
		value = "ZodleDY)7Tf0"; // Milestone
		mappingMetaAttributeObjectToMetaClass.put(key, value);	
		
		key = "tbFegCw7AXA0"; // ~tbFegCw7AXA0[Picture Identifier]
		value = "VkKHULaDr400"; // MetaPicture
		mappingMetaAttributeObjectToMetaClass.put(key, value);
			
		return mappingMetaAttributeObjectToMetaClass;

	}
	
	
	public static MegaCollection cleanCollectionMetaAttributeObject(MegaRoot megaRoot, MegaCollection megaCollection) {
		MegaCollection oCol = megaRoot.getSelection("");
		oCol.insert(megaCollection);
// the metattribute in this list should be treated as attribute and not as link.
//		oCol.remove(megaRoot.getObjectFromID("~jF1hiEhwFbLF[Skill Level]")); // 
//		oCol.remove(megaRoot.getObjectFromID("~YIX2JlGzM9L2[Corresponding Exchange]")); // Exchange Contract - 
//		oCol.remove(megaRoot.getObjectFromID("~sb2usLFgFDCI[Computed State]")); // Expense - 
//		oCol.remove(megaRoot.getObjectFromID("~D33MKbLxF11J[Data Language]")); // Login Holder - 
//		oCol.remove(megaRoot.getObjectFromID("~eIJDiNJFFrIP[Source Collection]")); // 
//		oCol.remove(megaRoot.getObjectFromID("")); // Workflow Subject - Workflow Subject System - 
//		oCol.remove(megaRoot.getObjectFromID("")); // Workflow Parallel Status Base - Workflow Parallel Status System - 
//		oCol.remove(megaRoot.getObjectFromID("")); // Workflow Parallel Status Base - Workflow Parallel Status System - 
//		oCol.remove(megaRoot.getObjectFromID("")); // TimeSpot - 
//		oCol.remove(megaRoot.getObjectFromID("")); // TimeSpot - 
//		oCol.remove(megaRoot.getObjectFromID("~Tl4ifGh1LndB[Link Answered Object]")); // Answer - 
//		oCol.remove(megaRoot.getObjectFromID("~UsqWsS8oRXeP[Test Case Object]")); // Test Case Execution Item - 
//		oCol.remove(megaRoot.getObjectFromID("~WBb1A5HLIv5A[Author Chat Message]")); // Chat Message - 
//		oCol.remove(megaRoot.getObjectFromID("~uSrBWMeQIzcO[Last AuthorPost]")); // Post - 
//		oCol.remove(megaRoot.getObjectFromID("")); // Milestone - 
//		oCol.remove(megaRoot.getObjectFromID("")); // Milestone - 
//		oCol.remove(megaRoot.getObjectFromID("~JJNWaG8AI9fA[Author]")); // Post - 
//		oCol.remove(megaRoot.getObjectFromID("~TUOYtiORGbG1[Current Validation]")); // Validation Candidate Object - 
//		oCol.remove(megaRoot.getObjectFromID("~dcuUxkt0Cv10[Initial Workflow Status Instance]")); // Workflow Instance - Workflow Instance Base - 
//		oCol.remove(megaRoot.getObjectFromID("~TsARrPt1CLK0[Current Workflow Status Instance]")); // Workflow Instance - Workflow Instance Base - 
//		oCol.remove(megaRoot.getObjectFromID("")); // Time Dependent Element - 
//		oCol.remove(megaRoot.getObjectFromID("~Inh5EWHLI9LO[Recipient  Chat Message]")); // Chat Message - 
//		oCol.remove(megaRoot.getObjectFromID("~DXzyqLFkQLjC[GDPR - Organization]")); // GDPR Operating Processing - 
//		oCol.remove(megaRoot.getObjectFromID("~QeTeezhoOnYA[Annotated field]")); // Review Note - 
//		oCol.remove(megaRoot.getObjectFromID("~EsTif2VSPje4[Notification Activator]")); // Notification - 
//		oCol.remove(megaRoot.getObjectFromID("~rX(SRRfKIzgL[Locale Currency]")); // Org-Unit - 
//		oCol.remove(megaRoot.getObjectFromID("~vzkccmPqR5G0[Test Case Last Analyzed Item]")); // Test Case Execution Item - 
//		oCol.remove(megaRoot.getObjectFromID("")); // Generic Object System - Generic Object - 		
		...
		oCol.remove(megaRoot.getObjectFromID("~720000000T40[Link Creator]")); // 
		oCol.remove(megaRoot.getObjectFromID("~920000000b40[Link Modifier]")); // 
		oCol.remove(megaRoot.getObjectFromID("~d20000000T60[Object MetaClass IdAbs]")); // Generic Object System - Generic Object - 
		oCol.remove(megaRoot.getObjectFromID("~)20000000z70[Reading access area identifier]")); // Generic Object System - Generic Object - 
		oCol.remove(megaRoot.getObjectFromID("~130000000580[MetaConcept]")); // ChangeItemSystemTechnical - ChangeItemDataTechnical - ChangeItemSystemPublic - ChangeItemDataPublic - ChangeItemSystem - ChangeItemData - 
		oCol.remove(megaRoot.getObjectFromID("~330000000D80[Source Repository]")); // ChangeItemSystemTechnical - ChangeItemDataTechnical - ChangeItemSystemPublic - ChangeItemDataPublic - ChangeItemSystem - ChangeItemData - 
		oCol.remove(megaRoot.getObjectFromID("~430000000H80[Object]")); // ChangeItemSystemTechnical - ChangeItemDataTechnical - ChangeItemSystemPublic - ChangeItemDataPublic - ChangeItemSystem - ChangeItemData - 
		oCol.remove(megaRoot.getObjectFromID("~630000000P80[Major IdAbs]")); // ChangeItemSystemTechnical - ChangeItemDataTechnical - ChangeItemSystemPublic - ChangeItemDataPublic - ChangeItemSystem - ChangeItemData - 
		oCol.remove(megaRoot.getObjectFromID("~730000000T80[Minor  IdAbs]")); // ChangeItemSystemTechnical - ChangeItemDataTechnical - ChangeItemSystemPublic - ChangeItemDataPublic - ChangeItemSystem - ChangeItemData - 
		oCol.remove(megaRoot.getObjectFromID("~J30000000D90[Previous Substituting MetaAttribute]")); // 
		oCol.remove(megaRoot.getObjectFromID("~r4GgQpwlF9RH[Scheduling Origin]")); // System Trigger - 
		oCol.remove(megaRoot.getObjectFromID("~GEW1(R0QGXvC[Execution Repository]")); // System Job - 
		oCol.remove(megaRoot.getObjectFromID("~t1H96jKnCX92[FEA Service Component Reused UPI]")); // 
		oCol.remove(megaRoot.getObjectFromID("~0B16GYbHQj(8[Last Capability Realization Assessment]")); // Enterprise Undertaking - 
		oCol.remove(megaRoot.getObjectFromID("~e816wYbHQX29[Last Logical Sotware Realization Assessment]")); // Enterprise Undertaking - 
		oCol.remove(megaRoot.getObjectFromID("~HA16ZZbHQX69[Last Business Agent Realization Assessment]")); // Enterprise Undertaking - 
		oCol.remove(megaRoot.getObjectFromID("~3GniqXRORPa3[Node MetaClass]")); // GraphSet Node Definition - 
		oCol.remove(megaRoot.getObjectFromID("~DN1yLcF2D9vK[Dispatch (System)]")); // Repository Snapshot System - 
		oCol.remove(megaRoot.getObjectFromID("~dM1ygYF2DXrK[Dispatch (Data)]")); // Repository Snapshot - 
		oCol.remove(megaRoot.getObjectFromID("~pUXBhdqMGT82[Locked Object]")); // Unlocking Request - 
		oCol.remove(megaRoot.getObjectFromID("~lq111JGYGH0R[Execution Profile]")); // System Job - 
		oCol.remove(megaRoot.getObjectFromID("~Ntn3Z5ZVJDzS[Referenced Table  PostgreSQL]")); // SQL Clause - 
		oCol.remove(megaRoot.getObjectFromID("~Oqn3k5ZVJX0T[Referenced Column  PostgreSQL]")); // SQL Clause - 
		oCol.remove(megaRoot.getObjectFromID("~3GYCQDP0BDN0[Owning Library]")); // Generic Object - 
		oCol.remove(megaRoot.getObjectFromID("~yH2yBeFTDTqM[Design task]")); // Repository Snapshot - 
		oCol.remove(megaRoot.getObjectFromID("~bX3edYUo51F0[ChangeItemVisibility]")); // ChangeItemSystemPublic - ChangeItemDataPublic - ChangeItemSystem - ChangeItemData - 
		oCol.remove(megaRoot.getObjectFromID("~Yhp7CLdRIPgO[PERIOD BEGIN COLUMN  Teradata]")); // SQL Clause - 
		oCol.remove(megaRoot.getObjectFromID("~aep7NLdRIrjO[PERIOD END COLUMN  Teradata]")); // SQL Clause - 
		oCol.remove(megaRoot.getObjectFromID("~Gep7SzYRIrmN[NORMALIZE COLUMN  TeraData]")); // SQL Clause - 
		oCol.remove(megaRoot.getObjectFromID("~pxpkV1CDSrY7[Affinity]")); // Command Group Specialization - 
		oCol.remove(megaRoot.getObjectFromID("~L(JqaLzq6D30[Monitoring Indicator Calculation Macro]")); // 
		oCol.remove(megaRoot.getObjectFromID("~Us4lhDa8NbGP[DataBase Id]")); // Profile Assignment - 
		oCol.remove(megaRoot.getObjectFromID("~0w4iyDTNQH4E[Default Implementing Component _ Surcharged]")); // Working Environment Template - 
		oCol.remove(megaRoot.getObjectFromID("~dGLgVcALAH40[Default Perimeter]")); // MetaTool - _Operator - 
		oCol.remove(megaRoot.getObjectFromID("~3h5R53tVPLxE[Default Mode]")); // 
		oCol.remove(megaRoot.getObjectFromID("~KsbsVvjUF5WL[Aggregation Macro]")); // 
		oCol.remove(megaRoot.getObjectFromID("~AGcxyx0QMTRE[Database Synchronization Trigger]")); // Database - 
		oCol.remove(megaRoot.getObjectFromID("~(3N4TRbpPT13[DataGovernance - Hopex Source Object Id]")); // GDPR Sub-Processing - 
		oCol.remove(megaRoot.getObjectFromID("~rC7OABckQrTH[Test Case Reference Item]")); // Test Case Execution Item - 
		oCol.remove(megaRoot.getObjectFromID("~jAuM4qbTSjcC[Report Graph Arc Group - Group On - Object or Field]")); // Report Graph Arc - 
		oCol.remove(megaRoot.getObjectFromID("~nE8kWrh7Jn8N[Shortcut Target]")); // SolMan Shortcut - 
		oCol.remove(megaRoot.getObjectFromID("~8VO7Dy2ZPHvR[Current Overloading]")); // Desktop Container - MEGA Parameterized Tool - 
		oCol.remove(megaRoot.getObjectFromID("~lruWiwPENzc5[Last Assessment]")); // Enterprise Transformation Portfolio - Enterprise Undertaking - Project Risk - Project - Idea - Ideation Campaign - Strategy Evaluation - 
		oCol.remove(megaRoot.getObjectFromID("~87vJF1b8NjR5[Default Desktop]")); // MEGA Application - 
		oCol.remove(megaRoot.getObjectFromID("~Bcf0Y7HJFTlQ[Workflow Condition]")); // 
		oCol.remove(megaRoot.getObjectFromID("~86RZwwedAf10[Triggering Transition]")); // Workflow Status Instance System - Workflow Status Instance Base - 
		oCol.remove(megaRoot.getObjectFromID("~XiCX6RuvE9nF[ImplementationMacro]")); // System Job - 
		oCol.remove(megaRoot.getObjectFromID("~V1jvgp)wRv)V[GDPR - Physical Archive Country]")); // GDPR Processing Physical Archive - Physical Archive Category - 
		oCol.remove(megaRoot.getObjectFromID("~(OTofGTMGHQL[Source Repository  Scheduled Transfer]")); // Transfer Template - 
		oCol.remove(megaRoot.getObjectFromID("~bQToXHTMGDTL[Target Repository]")); // Transfer Template - 
		oCol.remove(megaRoot.getObjectFromID("~8TD21mpFHrnV[System Trigger]")); // Transfer - 
		oCol.remove(megaRoot.getObjectFromID("~H0EhFvQjSj2J[Report Graph Arc Group - Drilldown On - Object or Field]")); // Report Graph Arc - 
		oCol.remove(megaRoot.getObjectFromID("~0UUxW7KSIffB[WITH JOURNAL TABLE  Teradata]")); // SQL Clause - 
		oCol.remove(megaRoot.getObjectFromID("~jYkPV4doCbB0[Reused Agency Component]")); // 
		oCol.remove(megaRoot.getObjectFromID("~oWERy47k5DJ0[Master Object]")); // System Associative Object - Associative Object - 
		oCol.remove(megaRoot.getObjectFromID("~2XERz47k59K0[Slave Object]")); // System Associative Object - Associative Object - 
		oCol.remove(megaRoot.getObjectFromID("~IXER(47k55L0[MetaAssociationEnd Origin]")); // System Associative Object - Associative Object - 
		oCol.remove(megaRoot.getObjectFromID("~tXERBv6k5r60[Associative Object]")); // 
		oCol.remove(megaRoot.getObjectFromID("~tbEuJRE8GnKE[DocPicture]")); // System Business Document - Business Document - 
		oCol.remove(megaRoot.getObjectFromID("~xv(u(IUKHna6[Connection Snapshot]")); // Profile Assignment - 
		oCol.remove(megaRoot.getObjectFromID("~(Alcaq9rKL0H[Execution Data Language]")); // System Job - 
		oCol.remove(megaRoot.getObjectFromID("~uXVIwfK6Bz00[Implementing Macro]")); // Diagram - 
		
		oCol.remove(megaRoot.getObjectFromID("~tcl5kdbMPnbK[System Component Picture]")); // Class of Infrastructure Component - 
		...
		oCol.remove(megaRoot.getObjectFromID("~TsiQ0nsjGXuA[Workflow Status Instance Picture Identifier]")); // Workflow Status Instance System - Workflow Status Instance Base - 
		oCol.remove(megaRoot.getObjectFromID("~wnufTnCsQfi8[GDPR - Risk AssessmentPicture]")); // DPIA Risk Assessment - 
		oCol.remove(megaRoot.getObjectFromID("~1JubVrPLAfG0[Variable Object Picture Identifier]")); // Resource Architecture - Organizational Resource - Class of Building Block - Artifact - Responsibility Assignment - 
		oCol.remove(megaRoot.getObjectFromID("~EMeJuBbAGfQJ[Illustrated Picture Identifier]")); // System Illustrated Object (To be removed) - Business Role - Profile - 
		oCol.remove(megaRoot.getObjectFromID("~GUsFV1aIPPe8[Sketching Item Picture Identifier]")); // Sketching Item - 
		oCol.remove(megaRoot.getObjectFromID("~XVsu6FETD1UJ[Architecture Use Picture Identifier]")); // 
		oCol.remove(megaRoot.getObjectFromID("~KrmvAg0ZA5V1[Artifact Component Picture Identifier]")); // Artifact Component - 
		oCol.remove(megaRoot.getObjectFromID("~BAXsksDvP1uL[Working Environment MetaPicture Identifier]")); // Working Environment - 
		oCol.remove(megaRoot.getObjectFromID("~NmXS0qAIAP35[Physical Asset Picture Identifier]")); // 

		oCol.remove(megaRoot.getObjectFromID("~nCH76(n5ELfO[Initiative MetaPicture Identifier]")); // Initiative - 
		oCol.remove(megaRoot.getObjectFromID("~nS2y7tF1QnD7[Regulator Picture]")); // Regulator - 		
		oCol.remove(megaRoot.getObjectFromID("~2Uwa(vvMPzVU[Capability Fulfillment MetaPicture Identifier]")); // Capability Fulfillment - 		
		oCol.remove(megaRoot.getObjectFromID("~T3vfqZw0CjB0[OrgUnit Component MetaPicture Identifier]")); // Org-Unit Component - 		
		oCol.remove(megaRoot.getObjectFromID("~pyj0qzWMPfl8[Referenced Building Block MetaPicture Identifier]")); // Idea Portfolio Line - Information Service Catalog Item - Business Catalog Item - Hardware Service Catalog Item - Technical Service Catalog Item - 
		oCol.remove(megaRoot.getObjectFromID("~lUUmVb4mOz0E[Portfolio Line MetaPicture Identifier]")); // Projects Portfolio Line - 
		oCol.remove(megaRoot.getObjectFromID("~fSUpNWVWOTZL[Project Delivrable MetaPicture Identifier]")); // Project Deliverable - 
		oCol.remove(megaRoot.getObjectFromID("~61VtrdpyP9OV[EA Project Deliverable MetaPicture Identifier]")); // EA Project Deliverable - 
		oCol.remove(megaRoot.getObjectFromID("~h)5gccwkF9KI[Deployment Context MetaPicture Identifier]")); // Usage context - 
...
/*
	private void removeInvalid(String absoluteIdentifier) {
		...
		if (absoluteIdentifier.contentEquals("5pBfewwrPXwD")) {
			//this.isValidMetaAttribute=false;
		} else if (absoluteIdentifier.contentEquals("fe92MzG)CPvG")) {
			//this.isValidMetaAttribute=false;			
		}  else if (absoluteIdentifier.contentEquals("LZzWljBeCDK8")) {
			//this.isValidMetaAttribute=false;			
		} else if (absoluteIdentifier.contentEquals("EZzWvcCeCjjA")) {
			//this.isValidMetaAttribute=false;			
		} else if (absoluteIdentifier.contentEquals("aB0EzoNA8zV0")) {
			//this.isValidMetaAttribute=false;			
		} else if (absoluteIdentifier.contentEquals("xXNpr2DzCzc0")) {
			//this.isValidMetaAttribute=false;			
		}
	...
	public static MegaCollection cleanCollectionMetaAttribute(MegaRoot megaRoot, MegaCollection megaCollection) {
		...
		//   oCol.remove(megaRoot.getObjectFromID("~KZjN4(b9FDnC[GRCOnline]"));
		 ...
		 //  oCol.remove(megaRoot.getObjectFromID("~2NaRYvdOHzL7[Workflow Current Status] ")); 
		   ...
		   oCol.remove(megaRoot.getObjectFromID("~I20000000950[MetaProtection] ")); 
		   oCol.remove(megaRoot.getObjectFromID("~UdHRDRN)4HD0[REPLACEMySQL] ")); 
		  // oCol.remove(megaRoot.getObjectFromID("~5pBfewwrPXwD[Depending Entities] ")); 
