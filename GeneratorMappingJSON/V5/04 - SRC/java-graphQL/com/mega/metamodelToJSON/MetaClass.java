...
//	private HashMap<String,String> metaAssociationEndHashMap = new HashMap<String,String>();	
	...
	private void setProperties() {
		...
		constraints.setReadOnly(isReadOnlyByIdAbs(oIdAbs));
	...
	private boolean isReadOnlyByIdAbs(String oAbsoluteIdentifier) {
		boolean readonly = false;
		
		if (listMetaClassReadonly().contains(oAbsoluteIdentifier)) {
			readonly=true;
		} else {
			readonly=false;
		}
...
*/	
	private void setMetaAssociation() {
		...
		if (queryNode==true) {
			...
			// we don't put relationship for in between object.
		}
	...
	public static boolean isFeaturedMetaClass(MegaRoot megaRoot, MegaObject oMetaClass) {
		...
		if (Arguments.getIsArchimate()) {
			// *********************************************************************************************
			// Archimate case to identify properly the featured MetaClass
			// *********************************************************************************************

			MegaObject oFoundArchimate = pMetaClassTarget.getCollection(StaticFields.upperClasses).get(StaticFields.featureArchimateMetaClass); 
			// we found it is a MetaClass Archimate
			if (oFoundArchimate.exists()) {
				MegaObject oFoundFeature = pMetaClassTarget.getCollection(StaticFields.upperClasses).get(StaticFields.featureArchimateConceptRelationShip); 
				// Archimate feature MetaClass are flagged featureArchimateConceptRelationShip = "~szjqNhD1Q5KT[Archimate Concept Relationship]
			
				if (oFoundFeature.exists()) {
					result = true;
				} else {
					result = false;
				}	
				Generator.logger.finest("MetaClass =" + oMetaClass.getProp(StaticFields.shortName) + " is Feature for Archimate result=" + result);
	...
	public static List<String> listMetaClassReadonly() {
		List<String> listMetaClass = new ArrayList<String>();
		listMetaClass.add("M20000000Q10"); // [Language]
		listMetaClass.add("b200)))vXxQC"); // [DiagramType]
		listMetaClass.add("y20000000o30"); //[Login]
		listMetaClass.add("M20000000Q10"); // [DBMSVersion]
		listMetaClass.add("4Mmf(XQJxe20"); // [DBMS Version]
