...
public class Arguments {
	...
	private static boolean isArchimate = false;
...
//	private static String logfolder="C:\\\\temp\\\\java\\\\";
	...
	private static Integer openEXmode = 2;
	...
	public static boolean getIsArchimate() {
		return isArchimate;
	}
	
	public static void setIsArchimate(boolean isArchimate) {
		Arguments.isArchimate = isArchimate;
	}
	...
	public static Integer getOpenEXmode() {
		return Arguments.openEXmode;
	}
	
	public static void setOpenEXmode(Integer openEXmode) {
		Arguments.openEXmode = openEXmode;
	}	
	...
	public static void setGlobalVariables(String[] args) throws ParseException {
		...
		options.addOption("archi", "archimate", false, "Adjust the behavior for Archimate Metamodel. The metamodel must be in the SystemDb");
		...
		Option openEXmode   = Option.builder("openEXmode")
				.optionalArg(true)
				.longOpt("openEXmode")
				.argName("openEXmode" )
				.hasArg()
				.type(Integer.class)
				.desc("Specify the version of connexion string 1: for old approach 2: for new one with Login/Password/Profile. Default 2")
				.build();				
			options.addOption(openEXmode);				
		...
		if(cmd.hasOption("h")) {
			...
			if(cmd.hasOption("archi")) {
				Arguments.setIsArchimate(true);
			}	
			if(cmd.hasOption("openEXmode")) {
				Arguments.setOpenEXmode(Integer.parseInt(cmd.getOptionValue( "openEXmode" )));
			}				
