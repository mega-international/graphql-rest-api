...
public class MetaAssociationEnd extends CommonAttributes {
	...
	public MetaAssociationEnd(MegaRoot megaRoot, MegaObject oMetamodel, MegaObject oStartingMetaClass, MegaObject oInitialTargetMetaClass, MegaObject oInitialMetaAssociationEnd, MegaObject oMetaAssociationEnd , MegaObject oMetaClass, MegaObject oMetaAssociation,boolean isFirst, HashMap<String,String> overrideNameList, HashMap<String,String> globalUniqueName) {
		...
		if (oMetaAssociationEnd.sameID(personAssignment.getID())) {
			...
			boolean isAvailableMEA = (boolean) oMetaAssociationEnd.callFunction("IsAvailable");
			boolean isAvailableMetaClass = (boolean) oMetaClass.callFunction("IsAvailable");			
	...
	private void setPropertiesPersonAssignment(RelationshipsJSON relationshipsJSON, boolean isFirst, boolean navigate, boolean nameForComplexPath, MegaObject oStartingMetaClass, MegaObject oMetaAssociationEnd,MegaObject oMetaAssociation , MegaObject oMetaClass, MegaObject oBusinessRole, MegaObject oMetaAssociationEndNext, MegaObject oMetaAssociationNext ,MegaObject oMetaClassNext ) {
		...
		relationshipsJSON.addPathToTargetJSON(pathToTargetJSON,technincalNameStartingMetaClass,true,"","","");
		...
		//String globalUniqueName = technincalNameMetaClassNext + "_" + technincalName + "_" + technincalNameMetaClass + "_" + technincalNameStartingMetaClass;		
		...
		if (overrideNameList.containsKey(objectFilterID)) {
			...
			//globalUniqueName = technincalNameMetaClassNext + "_" + overrideNameList.get(objectFilterID) + "_" + technincalNameMetaClass + "_" + technincalNameStartingMetaClass;		

		}

		String convertedObjectFilterShortName = UtilitiesMappingJSON.getTechnicalNameMetaClass(objectFilterShortName);
		
		relationshipsJSON.addPathToTargetJSON(pathToTargetJSONNext,technincalNameStartingMetaClass,false,technincalNameMetaClassCondition  , technincalNameMaeCondition, convertedObjectFilterShortName);
		
		//String implementInterface = "relationship";
//		relationshipsJSON.setImplementInterface(implementInterface);

/*		
		if (this.globalUniqueName.containsKey(globalUniqueName)) {
			System.out.println("Error globalUniqueName already exist" + globalUniqueName);
		}
		this.globalUniqueName.put(globalUniqueName, globalUniqueName);
		
		
		relationshipsJSON.setGlobalUniqueName(globalUniqueName);
*/
...
//		String idrelationshipsJSON = id +"_"+ idNext + "_" + idCondition+ "_" + objectFilterID;
	...
	private void setProperties(RelationshipsJSON relationshipsJSON, boolean isFirst, boolean navigate, boolean nameForComplexPath, MegaObject oStartingMetaClass, MegaObject oMetaAssociationEnd, MegaObject oMetaClass, MegaObject oMetaAssociation ) {
		...
		relationshipsJSON.addPathToTargetJSON(pathToTargetJSON,technincalNameStartingMetaClass,false,"","","");
...
/*	
	private void setPropertiesMetaAttribute(PathToTargetJSON pathToTargetJSON ) {

		
		List<PropertiesJSON> propertiesJSONList = new ArrayList<PropertiesJSON>();		
		pathToTargetJSON.setProperties(propertiesJSONList);
